-- Fix rpc_find_due_schedules_next_min to prevent early and duplicate calls
...
BEGIN
  ...
  WHERE 
    -- Tightened scheduling window: only trigger within 30 seconds before to 2 minutes after scheduled time
    (EXTRACT(EPOCH FROM NOW()) - ss.run_at_unix) BETWEEN -30 AND 120
    -- Enhanced duplicate prevention: check call_sessions first, then call_logs
    AND NOT EXISTS (
      SELECT 1 FROM call_sessions cs
      WHERE cs.relative_id = ss.relative_id
        AND cs.household_id = ss.household_id
        AND cs.call_type = 'in_app_call'
        AND cs.created_at::date = CURRENT_DATE
        AND (
          (ss.slot_type = 'morning' AND EXTRACT(HOUR FROM cs.created_at AT TIME ZONE ss.timezone) BETWEEN 5 AND 11) OR
          (ss.slot_type = 'afternoon' AND EXTRACT(HOUR FROM cs.created_at AT TIME ZONE ss.timezone) BETWEEN 11 AND 17) OR
          (ss.slot_type = 'evening' AND EXTRACT(HOUR FROM cs.created_at AT TIME ZONE ss.timezone) BETWEEN 17 AND 23)
        )
    )
    AND NOT EXISTS (
