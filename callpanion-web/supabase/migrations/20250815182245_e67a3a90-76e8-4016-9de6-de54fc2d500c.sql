-- Relax origin allowlist to support preview domains and localhost
CREATE OR REPLACE FUNCTION public.validate_origin_and_log(
  _endpoint text, _origin text, _ip_address text, _user_agent text, _request_data jsonb DEFAULT '{}'::jsonb
) RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  log_id uuid;
BEGIN
  -- Allow common preview domains and localhost in addition to explicit allowlist
  is_allowed :=
    _origin IS NULL OR
