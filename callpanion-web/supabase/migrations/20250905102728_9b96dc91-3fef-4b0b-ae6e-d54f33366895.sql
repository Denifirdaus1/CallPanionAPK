-- Fix timezone calculation with proper approach
...
BEGIN
  ...
  due_slots AS (
    ...
    SELECT 
      ...
      st.phone_number,
      EXTRACT(EPOCH FROM (
        (CURRENT_DATE AT TIME ZONE st.timezone + st.morning_time::time) AT TIME ZONE st.timezone AT TIME ZONE 'UTC'
      ))::bigint as run_at_unix
    FROM schedule_times st
    WHERE st.morning_time IS NOT NULL
      AND ABS(EXTRACT(EPOCH FROM (
        (CURRENT_DATE AT TIME ZONE st.timezone + st.morning_time::time) AT TIME ZONE st.timezone AT TIME ZONE 'UTC' - now_utc
    ...
    SELECT 
      ...
      st.phone_number,
      EXTRACT(EPOCH FROM (
        (CURRENT_DATE AT TIME ZONE st.timezone + st.afternoon_time::time) AT TIME ZONE st.timezone AT TIME ZONE 'UTC'
      ))::bigint as run_at_unix
    FROM schedule_times st
    WHERE st.afternoon_time IS NOT NULL
      AND ABS(EXTRACT(EPOCH FROM (
        (CURRENT_DATE AT TIME ZONE st.timezone + st.afternoon_time::time) AT TIME ZONE st.timezone AT TIME ZONE 'UTC' - now_utc
      ))) <= 300 -- within 5 minutes
    ...
    SELECT 
      ...
      st.phone_number,
      EXTRACT(EPOCH FROM (
        (CURRENT_DATE AT TIME ZONE st.timezone + st.evening_time::time) AT TIME ZONE st.timezone AT TIME ZONE 'UTC'
      ))::bigint as run_at_unix
    FROM schedule_times st
    WHERE st.evening_time IS NOT NULL
      AND ABS(EXTRACT(EPOCH FROM (
        (CURRENT_DATE AT TIME ZONE st.timezone + st.evening_time::time) AT TIME ZONE st.timezone AT TIME ZONE 'UTC' - now_utc
      ))) <= 300 -- within 5 minutes
  )
  SELECT 
    ds.schedule_id,
