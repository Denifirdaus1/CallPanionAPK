...
DO $$ BEGIN
  IF to_regclass('public.wellbeing_logs') IS NOT NULL THEN
    ...
    -- (optional) UPDATE/DELETE policies: enable only if app requires editing/removal from UI
  END IF;
END $$;
...
DO $$ BEGIN
  IF to_regclass('public.calls') IS NOT NULL AND EXISTS (
    ...
    -- Create a UNIQUE INDEX CONCURRENTLY (safe on large tables). Note: partial index avoids multiple NULL rows.
    -- If conversation_id is nullable, we keep the unique check only for non-NULL values.
    CREATE UNIQUE INDEX IF NOT EXISTS idx_calls_conversation_id_unique
      ON public.calls(conversation_id)
      WHERE conversation_id IS NOT NULL;

    -- If conversation_id is already NOT NULL and you prefer a constraint, you can bind the index:
    -- ALTER TABLE public.calls ADD CONSTRAINT calls_conversation_id_key UNIQUE USING INDEX idx_calls_conversation_id_unique;
  END IF;
END $$;
