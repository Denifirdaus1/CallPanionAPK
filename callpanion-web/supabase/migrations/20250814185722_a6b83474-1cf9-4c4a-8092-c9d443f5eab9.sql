-- Add GDPR consent tracking columns to existing tables if not already present
ALTER TABLE households 
ADD COLUMN IF NOT EXISTS gdpr_consent_status boolean DEFAULT false,
...
-- Create function to sync household creation to HubSpot
...
-- Create function to sync invite acceptance to HubSpot
...
-- Create triggers
DROP TRIGGER IF EXISTS trigger_sync_household_to_hubspot ON households;
CREATE TRIGGER trigger_sync_household_to_hubspot
  AFTER INSERT ON households
  FOR EACH ROW
  EXECUTE FUNCTION sync_household_to_hubspot();

DROP TRIGGER IF EXISTS trigger_sync_invite_acceptance_to_hubspot ON invites;
CREATE TRIGGER trigger_sync_invite_acceptance_to_hubspot
  AFTER UPDATE ON invites
  FOR EACH ROW
  EXECUTE FUNCTION sync_invite_acceptance_to_hubspot();

-- Create function to manually sync existing contacts (for testing)
