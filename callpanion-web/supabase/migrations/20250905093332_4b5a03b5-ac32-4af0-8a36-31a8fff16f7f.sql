...
-- Add unique constraint to cron_heartbeat
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'cron_heartbeat_job_name_unique'
    ) THEN
        ALTER TABLE cron_heartbeat ADD CONSTRAINT cron_heartbeat_job_name_unique UNIQUE (job_name);
    END IF;
END
$$;
