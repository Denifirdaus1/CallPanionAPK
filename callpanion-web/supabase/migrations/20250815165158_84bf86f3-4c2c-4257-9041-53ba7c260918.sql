...
-- Policy 5: NO DELETE access for anyone except super admins (already covered by policy 1)

-- Step 3: Add additional security logging for customer access attempts
CREATE OR REPLACE FUNCTION public.log_customer_access_attempt()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  -- Log any attempt to access customer data
  IF TG_OP = 'SELECT' THEN
    PERFORM log_security_event('customer_data_access', jsonb_build_object(
      'customer_id', OLD.id,
      'operation', TG_OP,
      'user_id', auth.uid(),
      'table', 'customers'
...
-- Add trigger to log customer access (for monitoring)
DROP TRIGGER IF EXISTS log_customer_access ON public.customers;
CREATE TRIGGER log_customer_access
  AFTER SELECT ON public.customers
  FOR EACH ROW EXECUTE FUNCTION log_customer_access_attempt();