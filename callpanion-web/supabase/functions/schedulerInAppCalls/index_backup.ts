...
// ===============================================
// This scheduler implements a 2-phase system:
...
async function queueNotificationWithDeviceInfo(
  ...
  try {
    ...
    if (!deviceError && devicePair) {
      ...
      console.log(`[Queue] Device found: platform=${platform}, tokens=${deviceToken ? 'FCM✓' : 'FCM✗'} ${voipToken ? 'VoIP✓' : 'VoIP✗'}`);
      ...
      console.log(`[Queue] No device pairing found for relative ${schedule.relative_id}`);
    }
    ...
    console.log(`[Queue] ✅ Queued notification for relative ${schedule.relative_id} - execution at ${schedule.scheduled_time}`);
    return { success: true };
...
serve(async (req) => {
  ...
  try {
    console.log('=== Enhanced schedulerInAppCalls triggered ===');
    ...
    console.log(`[Phase 1] Found ${schedulesToQueue?.length || 0} schedules to queue:`, schedulesToQueue);
