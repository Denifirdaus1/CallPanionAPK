...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'start') {
      console.log('Starting ElevenLabs WebRTC call for session:', sessionId);

      // Get WebRTC conversation token (consistent with device calls)
      ...
      console.log('Updating call log with conversation ID for web session');
      
      const { callLogId, conversationId } = await req.json();
      ...
      console.log('Ending ElevenLabs WebRTC call for session:', sessionId);

      const { conversationSummary, duration, outcome = 'completed' } = await req.json();
    ...
    console.error('Error in elevenlabs-webrtc-call function:', error);
    return new Response(JSON.stringify({ 
